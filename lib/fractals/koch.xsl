...
xmlns:kochCurve="kochCurve"
xmlns:kochSnowflake="kochSnowflake"
>

    <kochCurve:kochCurve />
    <xsl:variable name="kochCurve" select="document('')/*/kochCurve:*[1]" />
    <xsl:template name="kochCurve" match="*[namespace-uri()='kochCurve']">
        ...
        <xsl:element name="svg" xmlns="http://www.w3.org/2000/svg">
            ...
            <xsl:element name="path">
                ...
                <xsl:attribute name="d">
                    <xsl:text>M 0 </xsl:text><xsl:value-of select="$offset" /><xsl:text> L</xsl:text>
                    <xsl:call-template name="doKochCurve">
                        <xsl:with-param name="x1" select="0" />
                        <xsl:with-param name="y1" select="$offset" />
                        <xsl:with-param name="x2" select="$size" />
                        <xsl:with-param name="y2" select="$offset" />
