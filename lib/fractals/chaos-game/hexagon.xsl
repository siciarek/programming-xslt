...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:chaos-game-hexagon="chaos-game-hexagon">

    <chaos-game-hexagon:chaos-game-hexagon />
    <xsl:variable name="chaos-game-hexagon" select="document('')/*/chaos-game-hexagon:*[1]" />
    <xsl:template name="chaos-game-hexagon" match="*[namespace-uri()='chaos-game-hexagon']">
        ...
        <xsl:param name="angle" select="60" />
        <xsl:param name="seed" />

        <xsl:variable name="bx" select="$width div 2" />
        <xsl:variable name="by" select="$width div 2" />
        ...
        <xsl:variable name="v6">
            <xsl:call-template name="protate">
                <xsl:with-param name="angle" select="5 * $angle" />
                <xsl:with-param name="base_x" select="$bx" />
                <xsl:with-param name="base_y" select="$by" />
                <xsl:with-param name="point_x" select="$bx" />
                <xsl:with-param name="point_y" select="$by - $size" />
        ...
        <xsl:call-template name="do-chaos-game-hexagon">
            <xsl:with-param name="stage" select="$stage" />
            <xsl:with-param name="seed" select="$seed" />
			
			<xsl:with-param name="sides" select="6" />
            ...
            <xsl:with-param name="x6" select="substring-before($v6, ',')" />
            <xsl:with-param name="y6" select="substring-after($v6, ',')" />

        </xsl:call-template>

    </xsl:template>

    <xsl:template name="do-chaos-game-hexagon">
...
<!-- base hexagon vertices -->
        ...
        <xsl:variable name="vx">
            <xsl:choose>
                <xsl:when test="$rand &lt;= 1.0 * (1.0 div $sides)">
                    <xsl:value-of select="$x1" />
                </xsl:when>
                <xsl:when test="$rand &lt;= 2.0 * (1.0 div $sides)">
                    <xsl:value-of select="$x2" />
                </xsl:when>
                <xsl:when test="$rand &lt;= 3.0 * (1.0 div $sides)">
                    <xsl:value-of select="$x3" />
                </xsl:when>
                <xsl:when test="$rand &lt;= 4.0 * (1.0 div $sides)">
                    <xsl:value-of select="$x4" />
                </xsl:when>
                <xsl:when test="$rand &lt;= 5.0 * (1.0 div $sides)">
                    <xsl:value-of select="$x5" />
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$x6" />
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        ...
        <xsl:if test="$stage &gt; 0 and $tail">
            <xsl:call-template name="do-chaos-game-hexagon">
                ...
                <xsl:with-param name="x6" select="$x6" />
                <xsl:with-param name="y6" select="$y6" />
