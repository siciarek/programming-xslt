...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tree_f="tree_f">
...
<!--
	...
	Created at  : 2012-03-27 13:33:57+00:00
...
-->
	...
	<tree_f:tree_f />
	<xsl:variable name="tree_f" select="document('')/*/tree_f:*[1]" />
	<xsl:template name="tree_f" match="*[namespace-uri()='tree_f']">
	...
	<xsl:call-template name="tree_fX">
	...
	<xsl:template name="tree_fX">
		...
		<xsl:if test="$depth &gt; 0">		
		<xsl:call-template name="tree_fF">
    ...
    <xsl:template match="/">
		...
		<xsl:variable name="content">
             <xsl:call-template name="tree_f">
                 <xsl:with-param name="depth" select="7" />
                 <xsl:with-param name="angle" select="20.000000" />
