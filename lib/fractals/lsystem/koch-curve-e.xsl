...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:koch_curve_e="koch_curve_e">
...
<!--
	...
	File        : koch-curve-e.xsl
	Procedure   : koch_curve_e
	Created by  : Jacek Siciarek <siciarek@gmail.com>
	Created at  : 2012-03-27 11:54:56+00:00

	Description : Koch Curve E

-->

	<koch_curve_e:koch_curve_e />
	<xsl:variable name="koch_curve_e" select="document('')/*/koch_curve_e:*[1]" />
	<xsl:template name="koch_curve_e" match="*[namespace-uri()='koch_curve_e']">
	...
	<xsl:call-template name="koch_curve_eF">
	...
	<xsl:template name="koch_curve_eF">
    ...
    <xsl:template match="/">
		...
		<xsl:variable name="content">
             <xsl:call-template name="koch_curve_e">
                 <xsl:with-param name="depth" select="4" />
                 <xsl:with-param name="angle" select="90" />
                 <xsl:with-param name="size" select="10" />
                 <xsl:with-param name="xoffset" select="90" />
                 <xsl:with-param name="yoffset" select="-160" />
             </xsl:call-template>
         </xsl:variable>
