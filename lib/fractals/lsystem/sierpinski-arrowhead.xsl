...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sierpinski_arrowhead="sierpinski_arrowhead">
...
<!--
	...
	File        : sierpinski-arrowhead.xsl
	Procedure   : sierpinski_arrowhead
	Created by  : Jacek Siciarek <siciarek@gmail.com>
	Created at  : 2012-03-27 13:01:25+00:00

	Description : Sierpinski Arrowhead pg. 19

-->

	<sierpinski_arrowhead:sierpinski_arrowhead />
	<xsl:variable name="sierpinski_arrowhead" select="document('')/*/sierpinski_arrowhead:*[1]" />
	<xsl:template name="sierpinski_arrowhead" match="*[namespace-uri()='sierpinski_arrowhead']">
	...
	<xsl:call-template name="sierpinski_arrowheadYF">
	...
	<xsl:template name="sierpinski_arrowheadXF">
		...
		<xsl:call-template name="sierpinski_arrowheadXF">
			<xsl:with-param name="depth" select="$depth - 1" />
			<xsl:with-param name="angle" select="$angle" />
			<xsl:with-param name="size" select="$size" />
		</xsl:call-template>
	...
	<xsl:template name="sierpinski_arrowheadYF">
    ...
    <xsl:template match="/">
		...
		<xsl:variable name="content">
             <xsl:call-template name="sierpinski_arrowhead">
                 <xsl:with-param name="depth" select="6" />
                 <xsl:with-param name="angle" select="60.000000" />
                 <xsl:with-param name="size" select="8.000000" />
