...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:koch_curve_a="koch_curve_a">
...
<!--
	...
	File        : koch-curve-a.xsl
	Procedure   : koch_curve_a
	Created by  : Jacek Siciarek <siciarek@gmail.com>
	Created at  : 2012-03-26 21:42:00+00:00

	Description : Koch Curve A pg. 16

-->

	<koch_curve_a:koch_curve_a />
	<xsl:variable name="koch_curve_a" select="document('')/*/koch_curve_a:*[1]" />
	<xsl:template name="koch_curve_a" match="*[namespace-uri()='koch_curve_a']">
	...
	<xsl:call-template name="koch_curve_aF">
	...
	<xsl:template name="koch_curve_aF">
    ...
    <xsl:template match="/">
		...
		<xsl:variable name="content">
             <xsl:call-template name="koch_curve_a">
                 <xsl:with-param name="depth" select="3" />
                 <xsl:with-param name="angle" select="90" />
                 <xsl:with-param name="size" select="10" />
                 <xsl:with-param name="xoffset" select="224" />
                 <xsl:with-param name="yoffset" select="-32" />
             </xsl:call-template>
         </xsl:variable>
