...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cantor_set="cantor_set">
...
<!--
	...
	File        : cantor-set.xsl
	Procedure   : cantor_set
	Created by  : Jacek Siciarek <siciarek@gmail.com>
	Created at  : 2012-03-26 20:15:01+00:00
...
-->
	...
	<xsl:variable name="cantor_set" select="document('')/*/cantor_set:*[1]" />
	<xsl:template name="cantor_set" match="*[namespace-uri()='cantor_set']">
	...
	<xsl:call-template name="cantor_setAF">
	...
	<xsl:template name="cantor_setAF">
		...
		<xsl:call-template name="cantor_setBf">
			<xsl:with-param name="depth" select="$depth - 1" />
			<xsl:with-param name="angle" select="$angle" />
			<xsl:with-param name="size" select="$size" />
		</xsl:call-template>
	...
	<xsl:template name="cantor_setBf">
