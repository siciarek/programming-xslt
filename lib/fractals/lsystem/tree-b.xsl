...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tree_b="tree_b">
...
<!--
	...
	Created at  : 2012-03-27 13:27:11+00:00
...
-->
	...
	<tree_b:tree_b />
	<xsl:variable name="tree_b" select="document('')/*/tree_b:*[1]" />
	<xsl:template name="tree_b" match="*[namespace-uri()='tree_b']">
	...
	<xsl:call-template name="tree_bX">
	...
	<xsl:template name="tree_bX">
		...
		<xsl:if test="$depth &gt; 0">		
		<xsl:call-template name="tree_bF">
    ...
    <xsl:template match="/">
		...
		<xsl:variable name="content">
             <xsl:call-template name="tree_b">
                 <xsl:with-param name="depth" select="5" />
                 <xsl:with-param name="angle" select="22.500000" />
                 <xsl:with-param name="size" select="6.000000" />
