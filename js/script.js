...
Pre3d = (function() {
	...
	function M(X, Y) {
		return {
			x : X.e0 * Y.x + X.e1 * Y.y + X.e2 * Y.z + X.e3,
			y : X.e4 * Y.x + X.e5 * Y.y + X.e6 * Y.z + X.e7,
			z : X.e8 * Y.x + X.e9 * Y.y + X.e10 * Y.z + X.e11
		}
	}
	function c(at, ar) {
		...
		return new z(ax * ai + aw * ac + av * Y, ax * ah + aw * ab + av * X, ax
				* af + aw * aa + av * an, ax * ad + aw * Z + av * al + au, aq
				* ai + ap * ac + ao * Y, aq * ah + ap * ab + ao * X, aq * af
				+ ap * aa + ao * an, aq * ad + ap * Z + ao * al + am, ak * ai
				+ aj * ac + ag * Y, ak * ah + aj * ab + ag * X, ak * af + aj
				* aa + ag * an, ak * ad + aj * Z + ag * al + ae)
	}
	function o() {
	...
	function y(ab) {
		...
		return new z(ae * af - ad * aa, ad * ac - ag * ae, ag * aa - ac * af,
				0, X * aa - ae * Y, ae * Z - X * ac, ac * Y - Z * aa, 0, ad * Y
						- X * af, ag * X - ad * Z, Z * af - ag * Y, 0)
	}
	function A(aa, ab) {
		var X = ab.length;
		var Y = Array(X);
	...
	function m(X) {
		...
		if (this.ctx.setStrokeColor == null) {
			this.ctx.setStrokeColor = function Z(ae, ad, aa, ab) {
				var ac = [ Math.floor(ae * 255), Math.floor(ad * 255),
						Math.floor(aa * 255), ab ];
	...
	function S(aj, ae, ap, aa, ao, Z, an, Y, ag, am, af, al, ad, ak) {
		...
		var ai = (ag * (ak * ao - al * an) + am * (af * an - ad * ao) + (ad
				* al - af * ak)
				* ap)
				/ X;
		var ah = (ag * (ak * Z - al * Y) + am * (af * Y - ad * Z) + (ad * al - af
				* ak)
				* aa)
				/ X;
	...
	m.prototype.bufferShape = function L(aj) {
		...
		var al = c(this.camera.transform.getMatrix(), this.transform
				.getMatrix());
		var ak = y(al);
	...
	m.prototype.drawBuffer = function l() {
		...
		for ( var ac = 0; ac < ai; ++ac) {
			...
			var ah = ad.texture;
			if (ah !== null) {
				S(al, ah.image, aa.i0.x, aa.i0.y, aa.i1.x, aa.i1.y, aa.i2.x,
						aa.i2.y, ah.u0, ah.v0, ah.u1, ah.v1, ah.u2, ah.v2);
				if (!ae) {
					S(al, ah.image, aa.i0.x, aa.i0.y, aa.i2.x, aa.i2.y,
							aa.i3.x, aa.i3.y, ah.u0, ah.v0, ah.u2, ah.v2,
							ah.u3, ah.v3)
				}
			}
			...
			if (aj !== null) {
				...
				var ak = this
						.projectPointToCanvas(V(aa.centroid, w(aa.normal1)));
				al.beginPath();
				al.moveTo(Z.x, Z.y);
				al.lineTo(ak.x, ak.y);
				al.stroke()
			...
			if (ag !== null) {
				...
				var Z = this.projectPointToCanvas(aa.centroid);
				var ak = this
						.projectPointToCanvas(V(aa.centroid, w(aa.normal2)));
				al.beginPath();
				al.moveTo(Z.x, Z.y);
				al.lineTo(ak.x, ak.y);
				al.stroke()
	...
	m.prototype.drawPath = function s(ai) {
		var ae = this.ctx;
		var ad = c(this.camera.transform.getMatrix(), this.transform
				.getMatrix());
		var ac = this.projectPointsToCanvas(A(ad, ai.points));
		var ag = (ai.starting_point === null ? this.projectPointToCanvas(M(ad,
				{
					x : 0,
					y : 0,
					z : 0
				})) : ac[ai.starting_point]);
		ae.beginPath();
		ae.moveTo(ag.x, ag.y);
		var ah = ai.curves;
...
Pre3d.PathUtils = (function() {
	...
	function c(f, e, d) {
		return {
			x : e.x + e.x - 0.5 * (f.x + d.x),
			y : e.y + e.y - 0.5 * (f.y + d.y),
			z : e.z + e.z - 0.5 * (f.z + d.z)
		}
	}
	return {
		makeCircle : b,
		makeSpiral : a,
		fitQuadraticToPoints : c
	}
})();
DemoUtils = (function() {
	function c(i, h) {
	...
	function d(q, k, j, i, o, n, m, r) {
		...
		b(q.canvas, function(s) {
			...
			if (s.shift && s.ctrl) {
				q.camera.focal_length = g(0.05, 10, q.camera.focal_length
						+ (s.delta_y * 0.01))
			} else {
				if (s.shift) {
					h.z += s.delta_y * 0.01
...
window.addEventListener("load", function() {
	...
	var l = [ {
		n : "r",
		c : new Pre3d.RGBA(1, 0, 0, 1)
	}, {
		n : "g",
		c : new Pre3d.RGBA(0, 1, 0, 1)
	}, {
		n : "b",
		c : new Pre3d.RGBA(0, 0, 1, 1)
	}, {
		n : "a",
		c : new Pre3d.RGBA(0, 1, 1, 1)
	}, {
		n : "y",
		c : new Pre3d.RGBA(1, 1, 0, 1)
	}, {
		n : "w",
		c : new Pre3d.RGBA(1, 1, 1, 1)
	} ];
