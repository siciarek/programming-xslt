...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:include href="../lib/fractals/barnsley-fern.xsl" />

    <xsl:template match="/">

        <xsl:variable name="content">
        <xsl:call-template name="barnsley-fern">
            <xsl:with-param name="stage" select="//rand/@count" />
            <xsl:with-param name="width" select="512" />
            <xsl:with-param name="height" select="700" />
        ...
        <xsl:call-template name="image">
            ...
            <xsl:with-param name="stroke" select="'rgb(0, 128, 0)'" />
